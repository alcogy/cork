...
export async function createApp(ctx: ServiceCtx, data: { name: string; description?: string }) {
	const { db, env, user, request } = ctx;
	if (!data.name) return fail(400, { error: 'App name is required' });
	...
	await writeAuditLog({ db: env.DB, account_id: user.id, action: 'create', resource_type: 'app', resource_id: app.id, request });
...
export async function deleteApp(ctx: ServiceCtx, id: string) {
	...
	await writeAuditLog({ db: env.DB, account_id: user.id, action: 'delete', resource_type: 'app', resource_id: id, request });
	return { success: true };
}
...
export async function createRecord(ctx: ServiceCtx, appId: string, formData: FormData) {
	...
	await writeAuditLog({ db: env.DB, account_id: user.id, action: 'create', resource_type: 'app_record', resource_id: record.id, request });
	return { success: true };
}
...
export async function deleteRecord(ctx: ServiceCtx, recordId: string) {
	...
	await writeAuditLog({ db: env.DB, account_id: user.id, action: 'delete', resource_type: 'app_record', resource_id: recordId, request });
	return { success: true };
}
...
export async function togglePublish(ctx: ServiceCtx, appId: string) {
	...
	const app = await db.query.apps.findFirst({ where: eq(schema.apps.id, appId) });
	if (!app) return fail(404, { error: 'App not found' });
	await db.update(schema.apps).set({ is_published: !app.is_published }).where(eq(schema.apps.id, appId));
	return { success: true };
}
...
export async function saveAppDefinition(
	ctx: ServiceCtx,
	appId: string,
	data: { name: string; description: string; fields: string }
) {
	if (!data.name) return fail(400, { error: 'App name is required' });
	await ctx.db.update(schema.apps).set({
		name: data.name,
